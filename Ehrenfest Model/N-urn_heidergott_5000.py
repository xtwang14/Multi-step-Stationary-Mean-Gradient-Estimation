# -*- coding: utf-8 -*-
"""
Created on Thu Sep 23 19:44:52 2021
...
#Derivative transition kernel
def state_transition_p_plus_minus(urns, n_urn, N_ball, theta_list):
    urns_plus = urns.copy()
    urns_minus = urns.copy()
    # Select an urn to potentially move a ball from, weighted by number of balls
    urns_dist = urns / N_ball
    current_urn = np.random.choice(n_urn, p=urns_dist)
    #current_prob = urns_dist[current_urn]
    ...
    # Calculate transition probabilities to other urns and adjust by the current urn's theta
    target_urn_probs_plus= np.ones(n_urn) / (n_urn - 1)  # Uniform probability for other urns
    target_urn_probs_plus[current_urn] = 0  # Exclude the current urn itself for movement
    # Apply the specific theta for the current urn
    
    target_urn_probs_minus= np.zeros(n_urn)
    target_urn_probs_minus[current_urn] = 1
    
    # Select the target urn based on the adjusted probabilities
    new_urn_plus = np.random.choice(n_urn, p=target_urn_probs_plus)
    #new_urn_minus = np.random.choice(n_urn, p=target_urn_probs_minus)
    ...
    # Move the ball: decrease from current urn, increase in the new urn
    urns_plus[current_urn] -= 1
    urns_plus[new_urn_plus] += 1
    
    return urns_plus, urns_minus, current_urn

def state_transition_p_minus(urns):
    return urns



def compute_D(s, n_urn, N_ball, theta_list):
    transition_time = 0
    estimator = 0
    normalizing_constant = (n_urn - 1) / (n_urn)


    plus_chain, minus_chain, current_urn = state_transition_p_plus_minus(s, n_urn, N_ball, theta_list)
    grad_p = np.zeros(n_urn)
    #minus_chain = state_transition_p_minus(s)
    transition_time += 2
    
    estimator += cost_f(plus_chain, n_urn, N_ball) - cost_f(minus_chain, n_urn, N_ball)
    
    
    while np.any(plus_chain != minus_chain):
        #print('transition_time')
        #print(transition_time/2)
        #print(transition_time)
        #print('plus-minus {}'.format([plus_chain, minus_chain]))
        plus_chain, _, _, minus_chain, _, _ = couple_state_transition(plus_chain, minus_chain, n_urn, N_ball, theta_list)
        transition_time += 2
        
        estimator += cost_f(plus_chain, n_urn, N_ball) - cost_f(minus_chain, n_urn, N_ball)
    
    grad_p[current_urn] = estimator

    return grad_p * normalizing_constant, transition_time


        

def Phanthom_estimator_regen(s, N_iter, n_urn, N_ball, theta_list):
    estimator = 0
    estimator_tau = 0
    transition_time = 0
    starting_point = s.copy()
    ...
    for i in range(N_iter):
        ...
        #while True:
        s, _, _ = one_step_transition(s, n_urn, N_ball, theta_list)
        transition_time += 1
        
        esti, tt = compute_D(s, n_urn, N_ball, theta_list)
        estimator += esti
        estimator_tau += 1
        transition_time += tt
        #print('s is {}'.format(s))
        #print('starting_point is {}'.format(starting_point))
        #if np.any(s != starting_point) == False:
        #    break
        
    
    estimator = estimator / N_iter
    estimator_tau = estimator_tau / N_iter
    
    final_estimator = (estimator) / (estimator_tau)
    
    return [final_estimator[-1], 1, transition_time]
...
for i in range(iteration):
    ...
    st_old = Phanthom_estimator_regen(urns, N_iter, n_urn, N_ball, theta_list)
    ...
    #wv_sqrt_pop.append(np.sqrt(st_old[0] * st_old[1]))
        ...
        if n % batch_size == 0:
            ...
            filename = f"heidergott_N_{N_iter}.csv"
